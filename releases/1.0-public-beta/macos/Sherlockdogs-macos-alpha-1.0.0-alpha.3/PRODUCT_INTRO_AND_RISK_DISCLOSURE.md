...
手机分享到 iOS Shortcut
-> 用户自己的同步 Inbox
   OneDrive / iCloud Drive for Windows / Google Drive / Syncthing / NAS / 坚果云
-> Windows 本地后台任务
-> 本地 Markdown 图书馆
-> 可选 Codex 对话
```
...
| 是否读取个人微信数据库 | Mac Personal Mode 需要用户主动运行 `Sherlockdogs Connect WeChat.app`；Windows Alpha 不读取 Windows 微信 DB |
...
| Windows 手机入口 | 当前 Windows 依赖 iOS Shortcut/同步 Inbox，不是 Mac 同款微信 DB 抓取 | 发布说明里明确边界；补 Windows WeChat DB adapter 后再宣传同款 |
...
| Windows 不默认解码个人微信数据库 | Windows WeChat DB adapter 尚未实现和真机验证 |

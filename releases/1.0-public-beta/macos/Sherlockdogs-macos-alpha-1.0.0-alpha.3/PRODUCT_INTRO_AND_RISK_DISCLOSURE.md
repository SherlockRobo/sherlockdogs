...
| 工具 | 作用 |
|---|---|
| iCloud / 坚果云 / OneDrive / Google Drive / Syncthing | 用户自己的手机到电脑同步 Inbox，作为微信自聊之外的备用入口 |
...
| 是否读取个人微信数据库 | Mac Personal Mode 需要用户主动运行 `Sherlockdogs Connect WeChat.app`；Windows 需要用户主动运行 `2 OneClick Configure.cmd`，本地准备/绑定 Windows 微信 DB |

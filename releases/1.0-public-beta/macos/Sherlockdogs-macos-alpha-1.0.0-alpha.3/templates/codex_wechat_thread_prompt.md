...
这是 Sherlockdogs 后台自动任务。请静默处理微信文章：抓取原文、拆解 Markdown、下载图片、按任务蒸馏，并写入 Obsidian。处理中不要发送自然语言进度消息，只在全部完成后发送最终交付卡片。
...
```text
/Users/bytedance/ObsidianVault_LOCAL/00-dashboard/principles/vault-migration-2026-06-01-read-first.md
/Users/bytedance/ObsidianVault_LOCAL/schema/vault-principles-2026-05-30.md
/Users/bytedance/ObsidianVault_LOCAL/schema/path-routing.md
```

1. 使用本地脚本抓取文章：

```bash
python3 /Users/bytedance/ObsidianVault_LOCAL/writing/ops/x-xhs-wechat-ob-crawler/scripts/wechat_capture.py \
  --url {{JOB_URL}} \
  --task {{JOB_TASK}} \
  --job-id {{JOB_ID}} \
  --json
...
7. 不要写到 `/Users/bytedance/Documents/Codex/...` 作为最终产物；微信剪藏最终产物只能写到 `/Users/bytedance/ObsidianVault_LOCAL/clipping/wechat/`。

...
| 修复入口 | `Sherlockdogs OneTouchRepair.app` |
| 诊断入口 | `Sherlockdogs Doctor.app` |
| 输出入口 | `Sherlockdogs Open Output.app` |
| 兜底配置 | `Configure Nutstore Inbox.command` |

## 用户使用

1. 打开 `INSTALL_GUIDE_FOR_USERS.png`。
2. 双击 `Sherlockdogs Start.app`。
3. 双击 `Sherlockdogs Connect WeChat.app`，按提示用手机微信转发一条测试内容给自己。
4. 以后手机把链接、图片、PDF、视频、文本转发给自己的微信。
5. Mac 收到后自动入库；用 `Sherlockdogs Open Output.app` 查看结果。
6. 没进来先双击 `Sherlockdogs OneTouchRepair.app`；需要诊断再双击 `Sherlockdogs Doctor.app`。
7. 兜底：坚果云用户双击 `Configure Nutstore Inbox.command`，绑定自己的 `Sherlockdogs/Inbox`。

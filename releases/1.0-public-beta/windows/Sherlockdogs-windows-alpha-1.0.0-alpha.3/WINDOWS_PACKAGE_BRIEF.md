...
Windows 测试包正在补齐 Mac 同款产品路径：手机微信发给自己，Windows 微信收到，Sherlockdogs 读取本机 Windows 微信 DB 并入库。当前包已把用户入口收敛成四个一键：安装、配置、修复、上报；边界是真 Windows 机器还必须跑完整 self-chat smoke。
...
| 一键安装 | `1 OneClick Install.cmd` |
| 一键配置 | `2 OneClick Configure.cmd` |
| 一键修复 | `3 OneClick Repair.cmd` |
| 一键上报 | `4 OneClick Report.cmd` |
| 旧默认入口 | `Sherlockdogs Start.cmd` |
| 旧微信入口 | `Sherlockdogs Connect WeChat.cmd` + 本地 decrypt bootstrap / 已解密 Windows 微信 DB |
| 可选配置入口 | `Configure Nutstore Inbox.cmd` |
| 诊断入口 | `Doctor Sherlockdogs.cmd` |
| Obsidian 指引 | `OBSIDIAN_SETUP.md` |
| 输出入口 | `Open Sherlockdogs Output.cmd` |

## 用户使用

1. 打开 `INSTALL_GUIDE_FOR_USERS.png`。
2. 双击 `1 OneClick Install.cmd`。
3. 保持 Windows 微信登录。
4. 双击 `2 OneClick Configure.cmd`；它会选择 Obsidian vault / 输出路径，绑定已有解密 DB，或尝试本地 decrypt bootstrap。
...
| 运行状态 | 写入用户配置的 `clipping\_sherlockdogs` |
...
| `2 OneClick Configure.cmd` | 一键配置 Obsidian 输出路径和 Windows 微信 |

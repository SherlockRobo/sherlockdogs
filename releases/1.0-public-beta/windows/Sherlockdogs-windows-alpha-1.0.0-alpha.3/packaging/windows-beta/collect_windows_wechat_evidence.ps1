...
$AdapterOutput = if (Test-Path $TaskRunner) {
  & powershell -NoProfile -ExecutionPolicy Bypass -File $TaskRunner -Kind windows-wechat 2>&1
} else {
...
if ($ReceiverLine) {
  ...
  if ($ReceiverChat -and -not $NoDiscoverReceiver) {
    ...
    New-Item -ItemType Directory -Force -Path (Split-Path -Parent $ReceiverFile) | Out-Null
    [System.IO.File]::WriteAllLines($ReceiverFile, [string[]]@($ReceiverChat), [System.Text.UTF8Encoding]::new($false))

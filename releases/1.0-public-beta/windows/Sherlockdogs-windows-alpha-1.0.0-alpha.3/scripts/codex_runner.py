...
from sdogs_paths import CLIPPING_DIR, JOBS_DIR, PROJECT_DIR as DEFAULT_PROJECT_DIR, RUNS_DIR as DEFAULT_RUNS_DIR, VAULT_DIR  # noqa: E402
...
class AppServerClient:
    ...
    def _pump_stream(self, stream: Any, name: str) -> None:
        if not stream:
            return
        for line in stream:
            if name == "stderr":
    ...
    def _read_line(self, deadline: float) -> dict[str, Any] | None:
        while time.monotonic() < deadline:
            ...
            try:
                line = self._stdout_queue.get(timeout=0.2)
            except queue.Empty:
                continue
            try:

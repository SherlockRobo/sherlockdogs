...
Windows 测试包是给普通用户双击运行的本地版 Sherlockdogs。它不默认读取个人微信数据库，不使用 Sherlockdogs 云中转；默认使用本地 Inbox，也可以绑定用户自己的坚果云、OneDrive、Google Drive、Syncthing 或 NAS 同步目录，电脑端收到文件后在本机入库，并按指令进入 Codex 对话。
...
5. 手机把链接、图片、PDF、视频、文本通过快捷指令或同步盘发到自己的 Inbox。

...
7. 跑 `Collect Windows WeChat Evidence.cmd` 生成 smoke 证据。
...
Windows 包已通过本机静态 release gate 和模拟 decrypted DB adapter smoke。正式标成 Mac 同款前，还必须在真实 Windows 机器上验证：取 key / 解密 -> `Sherlockdogs Connect WeChat.cmd` -> 微信自聊消息入库 -> Codex 卡片 -> `Collect Windows WeChat Evidence.cmd` 产出 PASS。

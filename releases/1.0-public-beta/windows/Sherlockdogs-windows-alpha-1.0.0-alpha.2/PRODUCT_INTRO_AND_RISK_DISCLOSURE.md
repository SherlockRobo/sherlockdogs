...
手机微信发给自己
-> Windows 微信收到
-> Sherlockdogs Connect WeChat 读取已解密的本机 Windows 微信 DB
-> 本地 Markdown 图书馆
-> 可选 Codex 对话
```
...
| 是否读取个人微信数据库 | Mac Personal Mode 需要用户主动运行 `Sherlockdogs Connect WeChat.app`；Windows 需要用户主动运行 `Sherlockdogs Connect WeChat.cmd` 并选择已解密 DB |
...
| Windows 微信 DB | 当前已有 adapter，但取 key / 解密层仍需 Windows 真机验证 | 发布前做真实 Windows DB smoke，未通过前不标同款完成 |

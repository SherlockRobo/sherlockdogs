...
给用户只讲主路径：打开 Sherlockdogs，点 `Connect WeChat`，手机转发一条测试内容给自己的微信，之后继续转发给自己即可。Mac 读取本机 Mac 微信 DB；Windows 当前 alpha 读取已解密的本机 Windows 微信 DB。Inbox/Shortcut/同步盘只作为备选实验路径，不是 Windows 核心卖点；Obsidian 推荐但不强制；Codex 需要本机已安装或可用。

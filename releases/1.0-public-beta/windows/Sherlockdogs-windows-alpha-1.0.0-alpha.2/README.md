...
2. 保持 Windows 微信登录。
3. 双击 `Sherlockdogs Connect WeChat.cmd`；它会绑定已有解密 DB，或尝试本地解密 bootstrap。
4. 手机微信转发一条内容给自己的微信号。
...
- `Run Windows WeChat Smoke.cmd`：引导完整真机测试：连接、提示手机发 `#2` 给自己、自动采集证据
- `Collect Windows WeChat Evidence.cmd`：真实 `#2` 自聊测试后生成 Windows DB smoke 证据
